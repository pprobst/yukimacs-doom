...
** Agent-shell
Interactive shell for ACP-compatible coding agents (Claude, etc.).
Requires the external =claude-agent-acp= binary on =PATH=:
=npm install -g @agentclientprotocol/claude-agent-acp=.
Anthropic uses login-based authentication by default (no API key stored here).
#+BEGIN_SRC emacs-lisp :tangle yes
